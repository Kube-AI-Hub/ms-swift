...
> **4.5.x 新风险点**：上游 `hub_utils.py` 新增了 `patch_kernels()`，其内部调用 `safe_snapshot_download(repo_id, use_hf=False, allow_patterns=...)` 从 ModelScope 拉 kernel 仓库。我们的实现忽略 `use_hf` 后，kernel 仓库也会先试 CSGHub（大概率 404 再回退 MSHub），**功能可用但多一次失败请求**。若在意，可在 `safe_snapshot_download` 里对 `use_hf is False` 保留直连 MSHub 的旧路径——推荐移植时顺手处理：`use_hf` 显式为 True/False 时走 `get_hub(use_hf).download_model(...)`，仅 `None` 走级联。这同时也让下游显式指定后端的调用不受影响。
