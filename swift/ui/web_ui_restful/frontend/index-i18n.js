...
window.i18n = {
  zh: {
    ...
    sectionExportHub: '写回模型库',
    fieldPushToCsghub: '推送到模型库',
    fieldExistOk: '允许覆盖已有目录',
    hintExportHub: '勾选后将导出结果推送到模型库。仓库不存在会自动创建；微调实例已注入 Token 时可留空。请使用新的命名空间/仓库名，不要覆盖基座模型。',
    hintExportHubModelId: '格式：命名空间/仓库名，例如 admin/qwen3-lora-sft',
    hintExportAdapters: '合并 LoRA 时填写训练产出的 checkpoint 目录',
    exportHubModelIdRequired: '推送到模型库时必须填写模型库 ID（命名空间/仓库名）',
  ...
  en: {
    ...
    fieldDeviceMap: 'Device map',
    sectionExportHub: 'Write back to model library',
    fieldPushToCsghub: 'Push to model library',
    fieldExistOk: 'Overwrite existing output dir',
    hintExportHub: 'Push the export result to the model library. Missing repos are created automatically. Leave the token empty if the finetune instance already injected one. Use a new namespace/repo name; do not overwrite the base model.',
    ...
    exportHubModelIdRequired: 'hub_model_id (namespace/repo) is required when pushing to the model library',
